import { Polygon, Feature, Point, GeoJsonProperties, LineString, FeatureCollection } from 'geojson';
...
export function fetchArea(props: FetchAreaOptions): Promise<FetchAreaResult> {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (response && "data" in response) {
            ...
            let geoJSON = osmtogeojson(response.data, { flatProperties: false });
            const filtered = geoJSON.features
                // .map((f: Feature) => ({ ...f, properties: f.properties)}))
                //@ts-ignore
                .map((f: Feature) => {
            ...
            geoJSON.features
                .map(f => ({ ...f, properties: JSONPropertiesToXMLTags(f.properties as any, "_") }))
                .forEach((f: Feature) => {
                    const options = {
